...
#define TH_GENERIC_FILE "generic/PinHoleCameraProjectionBHWD.c"
#else

#include <stdbool.h>


static int nn_(PinHoleCameraProjectionBHWD_updateOutput)(lua_State *L)
{
  //THTensor *inputImages = luaT_checkudata(L, 2, torch_Tensor);
  //THTensor *grids = luaT_checkudata(L, 3, torch_Tensor);
  THTensor *points3D = luaT_checkudata(L,2, torch_Tensor)
  THTensor *output = luaT_getfieldcheckudata(L, 1, "output", torch_Tensor);

  int batchsize = points3D->size[0];
  int inputImages_height = points3D->size[1];
  int inputImages_width = points3D->size[2];
  int output_height = output->size[1];
  int output_width = output->size[2];
  int inputImages_channels = poin3D->size[3];

  int output_strideBatch = output->stride[0];
  int output_strideHeight = output->stride[1];
  int output_strideWidth = output->stride[2];

  int points3D_strideBatch = inputImages->stride[0];
  int points3D_strideHeight = inputImages->stride[1];
  int points3D_strideWidth = inputImages->stride[2];
